...
-- will be on the led 0 ready to bounce back. After the delay set with the switches elapses, in the next step the pwm_map will contain (2,1,0,1).
-- The element on the right, meaning the head of the tail, will dominates and will be on led 1. The next step will be (1,0,1,2) and the one after (0,1,2,3).
-- The procedure is: shift left discarding the element on the left while the first element on the right is the one that was there before the shift plus 1. When the number of leds
-- is reached, the direction must be changed and then we subtract one instead of summing. And so on.

-- The pwm signals, coming from a different module, are connected to the leds following the indexes contained in the pwm_map register. Highest duty cycle is connected to the head.
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
begin
	...
	begin
		if rising_edge(clk) then
			...
			-- needed to have a change of speed immediately, whitout waiting for the counter to finish when the SW are changed
            if sw /= sw_reg then
                counter_ms <= (others => '0');
                counter_clk <= 0; 
            end if;
			...
			-----------------------RESET LOGIC----------------------
			-- When reset = 1, all leds are turned on and the other registers initialized to '0'
			if (reset = '1') then
				dir <= '0';
				pwm_map <= (others => 0); 
